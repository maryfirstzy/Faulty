...
def base58_encode(b):
    """Encodes bytes to a Base58 string with leading zero preservation."""
...
def private_key_to_wif(priv_key_hex, compressed=True):
    """Converts a 64-character raw hex private key into Wallet Import Format (WIF)."""
    try:
        ...
        # 2. Add Compression Flag (0x01) if requested (starts with K or L)
        if compressed:
            extended_key += b"\x01"
        ...
        # 4. Append Checksum and Base58 Encode
...
def main(file_path="BTC.txt", output_path="Extract.txt"):
    ...
    print("[INFO] Executing high-speed key derivations and WIF conversions...")
    ...
    with open(output_path, "w", encoding="utf-8") as out:
        ...
        for index, item in enumerate(entries):
            ...
            # Solve the raw private key (Hex)
            cracked_key_hex = solve_private_key(
                data["R"], data["S1"], data["Z1"], data["S2"], data["Z2"]
            )

            # Convert the cracked Hex key into standard Compressed WIF format
    ...
    print(f"Successfully processed {total} entries into format outputs.")
    print(
        f"[SUCCESS] Solver complete. Raw hex and converted WIF keys exported to '{output_path}'."
    )
